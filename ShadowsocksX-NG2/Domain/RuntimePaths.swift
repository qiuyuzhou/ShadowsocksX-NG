...
/// 位置。目录 `v2/` 0700、文件 0600，由写入方与 wrapper 共同维护。
enum RuntimePaths {
  static func v2Directory() -> URL {
    FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
      .appendingPathComponent("ShadowsocksX-NG/v2", isDirectory: true)
  }

  /// sslocal 配置契约：激活派生的完整 JSON，GUI 原子写入、wrapper 以绝对路径
  /// 交给 `sslocal -c`（也是 SIGUSR1 热重载时上游重读的文件）。
  static func runtimeFileURL() -> URL {
    v2Directory().appendingPathComponent("sslocal-active.json")
  }
  ...
  static func agentPIDFileURL() -> URL {
    v2Directory().appendingPathComponent("agent.pid")
  }
