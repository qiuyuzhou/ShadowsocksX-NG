...
final class WorkspaceWindowOpeningAdapter: WorkspaceWindowOpening {
  ...
  /// 当前分区名（组合根经 bindTitle 接线）：开窗前先缓存，开窗时应用。
  ...
  /// 窗口标题接线（组合根一次性调用）：标题跟随当前 workspace destination
  /// 的分区名，随 workspace 内导航实时变化；订阅生命周期由本 adapter 持有。
  func bindTitle(_ updates: AnyPublisher<String, Never>) {
    titleCancellable = updates.sink { [weak self] title in
  ...
  private func makeWindow() -> NSWindow {
    ...
    //    标题（当前分区名）呈现在内容区前缘（scene 版外观）。缺省标题栏会把
    //    标题挤在红绿灯旁的窄条里。
