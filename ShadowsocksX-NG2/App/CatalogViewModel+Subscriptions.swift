...
/// 校验后同文档原子提交快照与刷新状态；任何失败保留最后成功快照、本地
/// enabled、活动目标与运行状态，仅标记来源失败。每次提交都触发 postCommit
...
extension CatalogViewModel {
  ...
  /// 删除订阅：递归清除订阅源、固定分组、远端成员与本地 overlay；活动目标
  ...
  private static func resolveCredentials(
    ...
    for child in group.children {
      ...
      case .server(let leaf):
        ...
        try credentials.save(leaf.record.password, for: passwordRef)
        var optionsRef: CredentialReference?
        if let options = leaf.record.pluginOptions {
        ...
        children.append(
          .group(
            try resolveCredentials(
              nested, catalogGroupID: nested.id, reuse: reuse, credentials: credentials)))
      }
    }
